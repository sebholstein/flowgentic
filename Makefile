.PHONY: build build-worker build-control-plane build-hookctl build-agentctl lint package proto proto-lint proto-clean sqlc clean run-worker run-worker-2 run-cp run-cp-2

build: lint build-worker build-control-plane build-hookctl build-agentctl ## Build all binaries
...
run-cp: build-worker ## Run the control plane (auto-starts embedded worker)
	PATH="$(CURDIR)/bin:$(PATH)" go run ./cmd/flowgentic-control-plane

run-cp-2: build-worker ## Run a second control plane on port 8090
