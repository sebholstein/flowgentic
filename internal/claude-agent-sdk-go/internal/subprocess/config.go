...
import (
	...
	"github.com/sebastianm/flowgentic/internal/claude-agent-sdk-go/internal/cli"
	"github.com/sebastianm/flowgentic/internal/claude-agent-sdk-go/internal/control"
	"github.com/sebastianm/flowgentic/internal/claude-agent-sdk-go/internal/shared"
)
...
func (t *Transport) SupportedCommands(ctx context.Context) ([]control.SlashCommand, error) {
	t.mu.RLock()
	defer t.mu.RUnlock()
	...
	if t.closeStdin {
		return nil, fmt.Errorf("SupportedCommands not available in one-shot mode")
	}
	if t.protocol == nil {
