...
func (p *Protocol) Initialize(ctx context.Context) (*InitializeResponse, error) {
	...
	if resultMap, ok := result.(map[string]any); ok {
		initResp.SupportedCommands = parseStringList(resultMap["supported_commands"])
		initResp.Commands = parseSlashCommands(resultMap["commands"])
		if len(initResp.Commands) == 0 {
			initResp.Commands = parseSlashCommands(resultMap["available_commands"])
		}
		if len(initResp.Commands) == 0 {
...
func parseSlashCommands(raw any) []SlashCommand {
	switch v := raw.(type) {
	case []string:
		out := make([]SlashCommand, 0, len(v))
		for _, name := range v {
			if name == "" {
				continue
			}
			out = append(out, SlashCommand{Name: name})
		}
		return out
	case []any:
		out := make([]SlashCommand, 0, len(v))
		for _, item := range v {
			...
			argumentHint, _ := cmdMap["argumentHint"].(string)
