...
- The worker injects a default Flowgentic MCP stdio server (`agentctl mcp serve`) when `AGENTCTL_WORKER_URL` and `AGENTCTL_AGENT_RUN_ID` are present in `LaunchOpts.EnvVars`.
